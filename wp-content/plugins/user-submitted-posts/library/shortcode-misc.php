...
function usp_gallery($attr, $content = null) {
	...
	foreach ($current_user_posts as $item) {
		...
		foreach ($images as $image) $gallery .= $image;
        	$gallery = $gallery ? '<div class="usp-image-gallery">'. $gallery .'</div><H1>'. $item->post_title .'</H1><br><a onclick="return confirm(\'Are you SURE you want to delete this post?\')"  href="'. get_delete_post_link($item->ID) .'"Delete</a><br><br>' : '';
		}

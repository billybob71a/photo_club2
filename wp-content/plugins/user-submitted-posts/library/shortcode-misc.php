...
function usp_gallery($attr, $content = null) {
	...
	foreach ($current_user_posts as $item) {
		...
		foreach ($images as $image) $gallery .= $image;
        	$gallery = $gallery ? '<div class="usp-image-gallery">'. $gallery .'</div><a onclick="return confirm(\'Are you SURE you want to delete this post?\')"  href="'. $item->ID .'</a><H1>'. $item->post_title .'</H1><br><br>' : '';
		}
